...
COURSERA_TOKEN = 'HtXg5ebXy6Swv3jx' ### YOUR TOKEN HERE
...
def make_model():
    ...
    model.add(Conv2D(filters=32, kernel_size=kernal_size, padding='same'))
    model.add(LeakyReLU(alpha))
    ...
    model.add(Conv2D(filters=32, kernel_size=kernal_size, padding='same'))
    model.add(LeakyReLU(alpha))
    model.add(Conv2D(filters=64, kernel_size=kernal_size, padding='same'))
    model.add(LeakyReLU(alpha))
    model.add(MaxPooling2D(pool_size=pool_size))
    model.add(Dropout(0.25))
...
for layer in conv_activation_layers:
    print(layer.name)
    plot_filters_stimuli(layer_name=layer.name, is_conv=True, model=model, verbose=False)
...
plot_filters_stimuli(layer_name=last_dense_layer.name, is_conv=False, 
                     iterations=200, step=0.1, model=model, verbose=True)
